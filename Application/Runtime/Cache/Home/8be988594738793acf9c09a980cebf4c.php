                            ...
                            <div class="adv-table editable-table ">
                                <div class="clearfix">
                                    <div class="btn-group">
                                        <button class="btn btn-primary">
                                            <a href="<?php echo U('Income/income_send');?>">新建计划表</a> <i class="fa fa-plus"></i>
                                        </button>
                                    </div><br>
                                ...
                                <table class="table table-striped table-hover table-bordered" id="editable-sample">
                                    ...
                                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="">
                                            <td><?php echo ($v["year_no"]); ?></td>
                                            <td><?php echo ($v["department_name"]); ?></td>
                                            <td><?php echo ($v["plan_man_name"]); ?></td>
                                            <td><?php echo ($v["plan_date"]); ?></td>
                                            <td class="center">
                                                <?php if($v["plan_flag"] == 0): ?>等待提交<?php endif; ?>
                                                <?php if($v["plan_flag"] == 1): ?>已提交待审核<?php endif; ?>
                                                <?php if($v["plan_flag"] == 2): ?>部门领导审核退回<?php endif; ?>
                                                <?php if($v["plan_flag"] == 3): ?>待财务审核<?php endif; ?>
                                                <?php if($v["plan_flag"] == 4): ?>财务审核退回<?php endif; ?>
                                                <?php if($v["plan_flag"] == 5): ?>财务审核通过<?php endif; ?>
                                            </td>
                                            <td><a class="edit" target="" href="<?php echo U('Income/income_detail');?>?id=<?php echo ($v['id']); ?>">查看明细</a></td>
                                            <!--<td><a class="edit" href="#" onclick="openwin(<?php echo ($v['id']); ?>)">查看明细</a></td>-->
                                            <td> <a style="cursor:pointer" href="<?php echo U('Income/send_shenhe');?>?id=<?php echo ($v["id"]); ?>">提交审核</a> </td>
                                            <td><a class="delete" onclick="del_com(<?php echo ($v['id']); ?>)">删除</a></td>
...
<script>
    ...
    function del_com(data){
        var r = confirm("确定删除吗？");
        if(r == true){
            window.location.href="income_del"+'?id='+data;
            return true;
        }else{
            return false;
        }
